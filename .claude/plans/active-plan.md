...
**Son güncelleme:** 2026-07-09 (Session 90 kapanış — 7 PR #315-321 · **ADİSYON FİŞİ REDESIGN CANLI** (#315 ADR-027 Amd1 48-kolon+parçalı döküm, API deploy) + **3 canlı bug fix** (#317 ödeme Kaydet footgun ADR-014 §13 · #318 sipariş-ekranı Kaydet layout · #316 web Yazdır butonu) + **CALLER ID** (#319 "Çağrılar" son-çağrılar modalı ADR-016 §11 + #320 çağrı→paket-sipariş) — hepsi web deploy'lu, kullanıcı canlı doğruladı + **ADR-033 Ödeme-Düzeltme (void+reopen) Accepted → Faz 1 BACKEND merge #321** (Migration 044 soft-void + atomik void→reopen + 13-SUM-filtre + gate'ler; **prod'a DEPLOY YOK** — Faz 2 frontend bekliyor). 2 sektör araştırması (caller-id modal + ödeme workflow). prod code `b8ea4b1`, main `a13cd8e`.)
**main HEAD:** `a13cd8e` (S90 kapanış; #315-321 merged) · **prod code == `b8ea4b1`** (#315-320 web/API DEPLOY'LU; **#321 ADR-033 Faz-1 backend + Migration 044 main'de ama prod'a DEPLOY YOK** — Faz 2 frontend olmadan UI yok + migration deploy=önce yedek)
**▶ SIRADAKİ: [KOD] ADR-033 Faz 2 = ödeme-void FRONTEND** (`ClosedOrdersPanel` satır "Geri Al" + split-state ödeme-satırı void + onay-modalı + sebep-dropdown enum [wrong_payment_type/wrong_amount/wrong_table/duplicate/other]; voided satır üstü-çizili; **hci-reviewer + turkish-ux ZORUNLU** [yıkıcı finansal aksiyon]; UI test; branch-first). Backend HAZIR (#321 `a13cd8e`: Migration 044 soft-void + `POST /payments/:paymentId/void` + `voidPayment` atomik void→auto-reopen + 13-SUM-fan-out voided-filtre; ADR-033 `decisions.md:11566`, DoD `:11664`). SONRA **migration+backend+frontend BİRLİKTE prod deploy** (migration'lı → önce `pg-backup.sh` yedek + `pm2 restart pos-api`; deploy.md §4). Sonra **MUTFAK fişi redesign** (S89'dan beri bekliyor, ayrı iş — Adisyo mutfak-fişi referansı istenecek). Opsiyonel ödeme **v5.0 quick-win** (Eşit-Böl N-kişi / denominasyon quick-cash ₺50/100/200 — S90 ödeme-workflow araştırması önerisi, mevcut modal/`/payments` ile). ⏸ ERTELENDİ: A4 KVKK hukuki + KDS + kasiyer-kiosk cutover + [ultracode 🔶] v5.1 derin denetim. Detay: `.claude/plans/session-91-kickoff.md`. **Açık chip:** mutfak em-dash+control-byte (`task_df442130`) · Kaydet-ödeme E2E (`task_4455260a`).**
