...
**Son güncelleme:** 2026-07-07 (Session 86 kapanış — **Caller Bridge readiness #291 (dotnet 12/12 + X-Tenant-Id kontrat doğrulandı + route-mount `/api` + bayat skill) + A4 KVKK paketi execute-hazır #292 ([ALANLAR] dolu + m.9/VERBİS güncel teyit; 🚩 Hetzner Türk-SCC engeli = avukat düğümü) + A2 ✅ (2 admin+1 garson, kara liste 0) + 🎯 A6 pilot çekirdek smoke ✅ (mobil→mutfak fişi + web kasiyer + mobil↔web senkron)**; KULLANICI KARARLARI: **KDS yok/kağıt fiş yeter** + **A4 hukuki pilotta ertelendi**)
**main HEAD:** `0253d00` (Session 86 docs #291/#292 merged) · **prod code == `2958e65` (#288) DEĞİŞMEDİ** — S86 işleri docs+C#+skill, prod runtime'ı etkilemez → deploy GEREKMEDİ
**▶ SIRADAKİ (Phase 5 kalan — AZALDI; hepsi [USER]/[OPS]/cutover): A5 Caller cihaz tipini DOĞRULA (USB-HID C812A vs seri-modem; opsiyonel, go-live bloke DEĞİL) + prod BRIDGE_TOKEN · kasiyer istasyonu (cutover) · cutover günü (kasa agent Zadig→WinUSB + codepage-scan 61 + order_no 1'den; ⛔ Adisyo yaşarken YASAK) · 2-4 hafta → Adisyo iptali = pilot bitiş. ⏸ ERTELENDİ/KAPSAM-DIŞI: A4 hukuki tesis (paket hazır) + KDS (kağıt fiş yeter). ✅ A1/A2/A3/A6/A7. Detay: `.claude/plans/session-87-kickoff.md`.**
...
| A5 | ~~KDS ekranı~~ + kasiyer istasyonu (kiosk) + Caller Bridge | [OPS] | **S86: KDS DÜŞTÜ** (kullanıcı: kağıt fiş yeter). Caller Bridge kodu **readiness ✅** (#291); kalan = donanım tipini DOĞRULA (USB-HID vs seri) + prod `BRIDGE_TOKEN` + install (**opsiyonel, bloke değil**). Kasiyer istasyonu cutover'da |
