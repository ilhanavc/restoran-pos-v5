...
**Son güncelleme:** 2026-07-04 (Session 81 — ADR-031 Accepted, Phase 5 planlaması; kod YAZILMADI, plan ADR'si)
**main HEAD:** `2f960b9` (Session 80 kapanışı #256) · **0 açık PR** · Session 81 = **ADR-031 docs PR (bu dosya + decisions.md)**
**▶ SIRADAKİ: P5-1 — Altyapı envanteri + provisioning (İLK ADIM = kullanıcı Hetzner/domain envanteri)**

## Durum: Phase 0-4 ✅ FİİLEN TAMAM · Phase 5 Pilot 🔄 PLANLANDI (ADR-031), implementasyon başlamadı
...
| **Phase 5 Pilot + Migration** | 🔄 **PLANLANDI (ADR-031)** — sprint listesi aşağıda |
...
### P5-1 — Altyapı envanteri + provisioning + env/secret (hedef: sunucu ayakta, DNS+SSL+socket yeşil)
- [USER] Hetzner hesabı + domain envanteri (var mı / satın al) — **İLK ADIM**
- [OPS] CX22 provision (Ubuntu 24.04) + BX11 Storage Box + firewall (PG 5432 internete AÇILMAZ)
- [OPS] PG17 + Node22 + PM2 (tek instance) + Nginx + Let's Encrypt kurulum (`hetzner-deployment` skill)
- [DOCS] `docs/ops/deploy.md` runbook: SSH+pull+build+migrate+restart · **prod env tam envanteri** (JWT_ACCESS/AGENT_SECRET, BRIDGE_TOKEN, TENANT_ID, DATABASE_URL/MIGRATOR_DATABASE_URL, WEB_ORIGIN, NODE_ENV=production, VITE_SOCKET_URL; `openssl rand`; E2E_BYPASS set edilmez) · **Nginx `/socket.io` upgrade bloğu** · ADR-001 §7.1 migrator DELETE-revoke doğrulaması buraya taşınır · migrator rotasyonu = sunucu-taraflı manuel adım (§7.2 amendment)
- **DoD:** `https://<domain>` TLS yeşil, API health 200, socket.io handshake OK, migration'lar sıfır DB'ye koştu (head 043)

### P5-2 — Prod bootstrap + KVKK inventory + veri taşıma (hedef: tenant hazır, müşteri defteri taşındı)
- [DOCS] `docs/compliance/kvkk-data-inventory.md` yaz (**PII taşınmadan ÖNCE** — go/no-go ön-koşulu; ADR-016 PR-8e drift kapanışı)
- [KOD] Prod bootstrap script (idempotent, `seed.ts` DEĞİL): tenant (name=fiş başlığı) + admin + tenant_settings (**yalnız timezone**) + ilk `agents` satırı (`generateAgentApiKey`/`hashAgentApiKey`, plaintext bir kez göster) — PR
...
- [OPS/USER] Menü + masa/bölge + kullanıcılar ELLE gir; kara liste kayıtları ELLE işaretle (`is_blacklisted` + reason)
- **DoD:** `TENANT_ID` env = bootstrap UUID; müşteri export=import satır sayısı, dedup doğrulandı; menü/masa/kullanıcı canlıda
...
1. **Hetzner hesabı/domain envanteri** (P5-1 İLK ADIM) — "almış olabilirim, emin değilim"
...
5. **Garson cihaz envanteri** — kaç Android, iOS var mı (K9 Android-only)
