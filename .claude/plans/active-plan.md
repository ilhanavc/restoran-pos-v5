...
- **Sprint 14 PR-5 — Web UI `/raporlar`** (5 alt-PR planı: 5a ✅ MERGED (#139, 2f10941) / 5b KPI tile+chart / 5c rapor tabloları / 5d CSV indir + Z/X butonları / 5e i18n + hci-reviewer + turkish-ux-reviewer). Backend %100 ✅ (Sprint 14 PR-2/3/4). **UI tutarlılık görevi (spawn_task chip)** — `<PageHeader>` standart component + 13 sayfa migrate; PR-5b'den önce yapılması önerilir (çift dokunuş engellenir). **ADR-014 comp/void domain** (orders.comped_at/void_at + emit + migration; Sprint 14d veya 15 öncesi, ADR amendment gerekebilir). **Sprint 13 s7-payment-mod-b.spec** lokal UI keşfi (skip kaldırma). Sıralı önerilen: **PageHeader tutarlılık → PR-5b → 5c → 5d → 5e → comp/void domain → Print Agent (Charter Phase 3 madde 4)**.

**Sprint 13 ✅ + Sprint 14 backend ✅ KAPANDI (Session 58, 2026-05-11, 12 PR #127-#137).** Sprint 13 ADR-014 §10 Mod B test örtüsü (5 integration + 3 e2e iskelet skip). Sprint 14 backend: ADR-015 Amendment 1 + ADR-021 ✅ Accepted, 5 yeni rapor endpoint (category-sales/anomalies cancel-only/user-performance/daily-close/snapshot) + CSV export 13/13 endpoint (?format=csv + UTF-8 BOM + ; delimiter + audit_logs entry + 100k row cap REPORT_TOO_LARGE). PR-4b1'de 3 ardışık CI fix (event_type 2-segment / FK cleanup chain / pool.end double-close). reports.test 12→93 (+81 test), pii-mask + csv-stream +52 unit test. Charter Phase 3 madde 5 backend %100, UI ⏳ PR-5b/c/d/e devam. main HEAD `2f10941` (PR-5a merged 2026-05-11, Session 59).
