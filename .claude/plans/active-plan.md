...
**Son güncelleme:** 2026-07-08 (Session 88 kapanış — 🎯 **ADR-004 Amd4 spooler transport UÇTAN UCA İMPLEMENTE**: 2 PR (#311 PR-A = TS core [config 3.dal + `spooler-transport.ts` + exhaustive dispatch] + C# NativeAOT winspool helper + vendored `spooler-raw.exe` 1.25MB + 39 test + **gerçek winspool P/Invoke smoke** [OpenPrinter→exit2] + Amd3 label kaldır; #312 PR-B = MSI `SpoolerHelperExe` component + `install-second-agent.ps1 -PrinterName` + msi.yml verify + **runbook §2 Zadig→spooler** + README). MSVC Build Tools kuruldu (winget, 2. denemede UAC onaylı). security+qa gate ✅. **Kasa printing artık Zadig'siz HAZIR** → cutover'ın en riskli adımı yapısal elendi. Önceki S87 — kasa spooler canlı doğrulandı + Amd4 Accepted #309.)
**main HEAD:** `eef69e6` (S88 = 2 PR #311-312; S87 sonu `8aa01d6` = 3 PR #307-309) · **prod code == `ad14c5a`** (S88'de deploy YOK — agent-binary/installer/docs/test; prod API runtime etkilenmez)
**▶ SIRADAKİ: [USER]/[OPS] CUTOVER GÜNÜ** (kod TAMAM — kasa printing Zadig'siz): kasiyer kiosk + test verisi temizliği + `order_no` 1'den + **yeni MSI'yı restoran PC'sine kur** (spooler-raw.exe agent-sibling geldi; eski kurulumda YOK → yeni MSI build+kur ya da exe elle `%PROGRAMFILES%\Restoran POS\Print Agent\`'a) + `install-second-agent.ps1 -PrinterName "KASA-2026" -JobKinds bill -ApiUrl … -ApiKey …` + kasa fiş smoke (Adisyo round-trip) + go-live smoke + go/no-go (p95<200ms); runbook `docs/ops/cutover-gunu-runbook.md` §2 (spooler'a güncel). 2-4 hafta → Adisyo iptali = pilot bitiş. ✅ A1/A2/A3/A5/A6/A7 + **kasa spooler impl (S88)**. ⏸ ERTELENDİ: A4 hukuki + KDS + P5-6 gate + [D ultracode 🔶] v5.1 denetim. Detay: `.claude/plans/session-89-kickoff.md`.**
...
- ⏳CUTOVER [OPS] **KASA agent — SPOOLER (Zadig'siz; ADR-004 Amd4 #311/#312 PROD-hazır):** kasa yazıcısı (POS-80, Windows queue `KASA-2026`, Adisyo-paylaşımlı) → **Zadig GEREKMEZ** (winspool RAW aynı sürücüden; sürücü değişmez, Adisyo bozulmaz — S87 round-trip doğrulandı). Kod+MSI+vendored exe TAM hazır (S88): `install-second-agent.ps1 -PrinterName "KASA-2026" -JobKinds bill` spooler config TAM yazar; codepage 61 S87'de fiziksel teyitli. Kalan (cutover günü): yeni MSI kur (spooler-raw.exe agent-sibling) + kasa fiş smoke
