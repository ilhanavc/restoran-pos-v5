...
- **Sprint 14 PR-5 ✅ TAMAMEN KAPANDI** (7/7 alt-PR + PR-6 + PR-7 cleanup). **Sprint 15 PR-1/2/3/4 ✅ KAPANDI** (range parametre feature backend → frontend → panel plumbing → CSV range). **Anomaly comp/void scope ✅ ADR-015 Amendment 3 Accepted (2026-05-13, Session 61)** — cancel-only kısıtı kaldırıldı, scope: cancel+comp+void (item-level comp + future-proof void), domain emit eklenmedi (kapsam kilidi). **Sırada: implementer Amendment 3 uygulaması** (anomalies.ts compute 3 sorgu birleşik + ~6 yeni integration test). Sonra **Sprint 13 s7-payment-mod-b.spec** lokal UI keşfi (skip kaldırma), sonra Print Agent (Charter Phase 3 madde 4, v3 StoreBridge sıfırdan rewrite).

**Sprint 13 ✅ + Sprint 14 backend ✅ KAPANDI (Session 58, 2026-05-11, 12 PR #127-#137).** Sprint 13 ADR-014 §10 Mod B test örtüsü (5 integration + 3 e2e iskelet skip). Sprint 14 backend: ADR-015 Amendment 1 + ADR-021 ✅ Accepted, 5 yeni rapor endpoint (category-sales/anomalies cancel-only/user-performance/daily-close/snapshot) + CSV export 13/13 endpoint (?format=csv + UTF-8 BOM + ; delimiter + audit_logs entry + 100k row cap REPORT_TOO_LARGE). PR-4b1'de 3 ardışık CI fix (event_type 2-segment / FK cleanup chain / pool.end double-close). reports.test 12→93 (+81 test), pii-mask + csv-stream +52 unit test. Charter Phase 3 madde 5 (Raporlar) ✅ TAM (backend + frontend + range). main HEAD `afa73f2` (Session 60 KAPANDI 2026-05-12, 11 PR: #146-#156). Bugünün özeti: Sprint 14 PR-5 7/7 + PR-6/PR-7 cleanup + Sprint 15 PR-1/2/3/4 (range feature) + ADR-015 Amendment 2 Accepted + 3 yeni shadcn primitive (Popover/Calendar/Tooltip).
