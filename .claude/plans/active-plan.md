...
- **Sprint 14 PR-5 ✅ TAMAMEN KAPANDI** (7/7 alt-PR + PR-6 + PR-7 cleanup). **Sprint 15 PR-1/2/3/4 ✅ KAPANDI** (range parametre feature backend → frontend → panel plumbing → CSV range). **Sırada: ADR-014 comp/void domain** (anomaly endpoint void/comp scope; şu an cancel-only). Sonra Print Agent (Charter Phase 3 madde 4, v3 StoreBridge sıfırdan rewrite). **Sprint 13 s7-payment-mod-b.spec** lokal UI keşfi (skip kaldırma) hâlâ açık.
