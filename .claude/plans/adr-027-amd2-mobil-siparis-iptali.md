...
**Onay akışı — çift onay DEĞİL, tek "sebep ekranı":** sheet'ten seçilince tam-genişlik onay adımı açılır: başlık, "emin misiniz" gövdesi, **sebep chip'leri** (≥44pt), en altta kırmızı "İptal Et" (sebep seçilene kadar pasif) + "Vazgeç". Gerekçe: ADR-027 K3 "hafif onay dialog'u" kararıyla uyumlu; sebep seçimi zaten ikinci bilinçli dokunuş = çift onaydan daha iyi (kasıt kanıtı üretir, boş "Emin misiniz?" üretmez). Başarı → `onPaid()` tetiklenir (masa boşaldı, Order ekranı geri gider — `mergeTable` emsali).

**K8a — WEB PARİTESİ: kalem sayısı + mutfak uyarısı KALDIRILDI (ürün sahibi, 2026-07-20).** İlk sürümde onay ekranı ek olarak "N ürün iptal edilecek" özeti ve "N ürün mutfağa gitti — hazırlanmışsa çöpe gidecek" uyarısı gösteriyordu. Ürün sahibi mobil ile web iptal ekranının **birebir aynı** olmasını istedi ("webdeki 3 noktadaki sipariş iptali ile aynı olsun") → iki ekstra blok çıkarıldı; mobil artık web dialogunun taşıdığı bilgiyi taşır (başlık · emin-misiniz gövdesi · sebep · iki buton). **Tekrar istenirse İKİ TARAFA birden eklenir**, tek tarafa değil — ayrışmanın kaynağı buydu. Yan etki (bilinçli): kaldırılan uyarı "mutfağa gitmiş ürün çöpe gider" bilgisini veriyordu; bu bilgi artık ekranda yok, riski K3 para kapısı + audit taşıyor.

**K8b — Hata SHEET İÇİNDE gösterilir, toast ile DEĞİL (canlı test bulgusu, 2026-07-20).** İlk sürüm reddi toast ile bildiriyordu; sheet bir `Modal` ve React Native'de modal her şeyin üstünde çizildiği için **toast modalın arkasında kalıp görünmüyordu** → ürün sahibi canlı testte "iptal et'e basınca hiçbir şey olmuyor" diye bildirdi. Hata artık sheet'in içinde kırmızı kutuda; sheet açık kalır (kullanıcı vazgeçebilsin veya tekrar deneyebilsin), yeni denemede temizlenir. **Ders:** modal içindeki akışların hata geri bildirimi modalın İÇİNDE olmalı.
