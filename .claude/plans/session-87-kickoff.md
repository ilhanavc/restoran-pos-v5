...
## DURUM
main **`d7a3972`**, **prod code `ad14c5a`** (#300/#302/#303 Caller ID API fix'leri deploy'lu), 0 açık PR. Session 86 = **14 PR (#291-304)**. **🎯 CALLER ID UÇTAN UCA CANLI** — dükkan PC kurulumu + canlı tam denetim **4 prod bug** buldu-düzeltti (#300 input offset · #301 router io · #302 emit Z-normalize · **#303 KÖK NEDEN: `callerStationLookup` bootstrap wiring**) + istasyon=admin İlhan + #304 regresyon kilidi (realtime.test 15/15). Kanıt: log zinciri skipping→ZodError→**emitted** + kullanıcı popup'ı gördü. Detay: anchor §2.
...
- **A5 ✅ CALLER ID CANLI (S86 finali):** dükkan PC `C:\restoran-pos\caller-bridge` servis Running; SetEvents gerçek çağrıda kanıtlandı; istasyon = **admin İlhan** (`tenant_settings.caller_id_station_user_id`; **istasyon değişirse yeni kullanıcı web'de hard-refresh etmeli** — oda join'i handshake'te). Küçük artıklar (bloklamaz): maskeli-no bypass canlıda denenmedi (unit-testli) · servis-restart auto-start smoke · Serilog relative path → loglar `C:\Windows\System32\logs\` (kozmetik; istenirse dükkan PC appsettings'inde absolute path). ⚠️ **single-file publish YASAK** (appsettings'i gömer → `ApiBaseUrl is required` çöküşü; README/runbook düzeltildi #299).
