...
--      immutability ayrı trigger'la korunur (payments_created_at_immutable).
```
...
**B3 — `payments_timing_check` clause daraltıldı + ayrı immutability trigger:**

Bulgu: §10.4.4'teki `payments_timing_check` trigger tanımı `BEFORE INSERT OR UPDATE OF created_at ON payments`. `payments.created_at` §7 snapshot disiplini gereği immutable olmalı. `UPDATE OF created_at` clause'u ya dead code (hiç tetiklenmez → yanıltıcı) ya da bir gün UPDATE izni verilirse backdoor.

Karar: **`payments_timing_check` artık yalnız `BEFORE INSERT`.** `payments.created_at` immutability için ayrı trigger `payments_created_at_immutable` eklendi — §7 snapshot invaryantının `payments` tablosuna uygulanması.
...
CREATE TRIGGER payments_created_at_immutable
...
**10.5.2 — Bekleyen CONCERN'ler (bucket'lanmış):**

**Bucket A — Must-fix-before-Bölüm 11 (yakın-blocker):**

- **C1 — `payment_items` junction'a `is_comped=true` kalem eklenmesi yalnız domain katmanında enforce ediliyor.** §10.2 "DB defansif" ilkesine aykırı; domain bypass edilirse ikram edilmiş kalem ödeme junction'ına girebilir, double-charge riski. Fix: `BEFORE INSERT ON payment_items` trigger — parent `order_items.is_comped=false` kontrolü. Bölüm 11 açılışından önce mini-pass'te eklenir.

**Bucket B — Bölüm 11 öncesi ayrı pass:**

- **C2 — `payment_items` UNIQUE konvansiyonu §6.2 prefix ilkesiyle netleştirilmeli.** Mevcut `UNIQUE (order_item_id)` — `tenant_id` prefix'i yok. Ya §6.2'ye "globally-unique UUID PK'ler için istisna" notu eklenir, ya da `UNIQUE (tenant_id, order_item_id)` formuna çevrilir. Karar mini-pass'te.
- **C3 — Trigger naming konvansiyonu tek formata çekilir.** Mevcut: `orders_propagate_full_comp`, `order_items_recompute_comp_amount`, `orders_block_comp_rollback`, `payments_sum_check`, `orders_sum_check_on_close`, `payments_timing_check`, `payments_created_at_immutable`, `orders_block_comp_on_closed`, `order_items_block_comp_on_closed`. Karışık iki form (`<table>_<verb>` vs `<table>_<noun>_<qualifier>`). Öneri: `<table>_<action>[_<when>]` tek form.
- **C4 — `propagate_full_comp` UPDATE'ine tenant filtresi eklenir** (`AND tenant_id = NEW.tenant_id`). Composite FK yeterli referansiyel güvence sağlar ama §6.3.1 defense-in-depth ilkesi mekanik ucuz düzeltmeyi gerektirir.
...
1. **§10.4.4 — `payments_timing_check` trigger clause daraltıldı:** `BEFORE INSERT OR UPDATE OF created_at ON payments` → `BEFORE INSERT ON payments`. B3 kararı uygulaması. Eski clause dead code / backdoor riskiydi; immutability artık ayrı `payments_created_at_immutable` trigger'ıyla kilitli (SQL §10.5.1 B3).
...
Başka §10.1-10.4 gövdesine dokunulmadı. CONCERN'lerin uygulanması §10.5.2 Bucket A/B sırasına göre **Bölüm 11 öncesi ayrı pass**'te yapılır — o pass'in diff'i ayrı commit olur (§10.5.2 C1'den başlayarak C2/C3/C4).

**10.5.5 — Bu gate sonrası `active-plan.md` Follow-up'a eklenecek borçlar:**

- **"Bölüm 11 öncesi mini-pass (CONCERN bucket A+B)":** C1 (DB guard for comped item in payment_items) + C2 (UNIQUE konvansiyon) + C3 (trigger naming) + C4 (propagate_full_comp tenant filter). Ayrı commit, Bölüm 11 draft başlamadan önce kapatılır.
