...
- **🎯 SONUÇ (S86 finali): CALLER ID UÇTAN UCA CANLI.** Donanım smoke GEÇTİ (SetEvents gerçek çağrıda çalıştı); canlı tam denetim **4 prod bug** buldu-düzeltti (#300 input offset · #301 router io · #302 emit Z · **#303 `callerStationLookup` bootstrap = kök neden**) + #304 regresyon kilidi. İstasyon = admin İlhan. Kalan küçük artıklar (bloklamaz): maskeli-bypass canlı denemesi (unit-testli) · servis-restart smoke · Serilog System32\logs kozmetiği (absolute path istenirse) · dist zip'ine karışan eski dev-test logu (dükkan PC `caller-bridge\logs\` içindeki 20:29 damgalı dosya — silinebilir).
