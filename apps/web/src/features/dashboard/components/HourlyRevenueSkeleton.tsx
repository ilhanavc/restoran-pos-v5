 ...
 * SVG ile pure-tailwind, recharts/echarts dep eklemeden minimal skeleton.
 * 21 saat (06:00–02:00) eksenli bar grid. Veri yok → tüm bar'lar 0,
 * gradient sweep animasyonu "yapım aşamasında" hissi verir.
 */
  ...
  return (
    <div className="relative">
      ...
      <div className="relative h-[200px] overflow-hidden rounded-lg bg-stone-50/60">
        {/* Sweep gradient — gentle "loading" feel */}
        <div
          aria-hidden="true"
          className="absolute inset-0 -translate-x-full animate-[shimmer_2.5s_ease-in-out_infinite] bg-gradient-to-r from-transparent via-amber-200/30 to-transparent"
        ...
        <div className="absolute inset-x-3 bottom-0 flex items-end justify-between gap-1 pb-2">
          {Array.from({ length: 21 }).map((_, i) => (
            <div
              key={i}
              className="w-full max-w-[12px] rounded-t bg-stone-200/60"
              style={{ height: '4px' }}
            />
          ))}
        </div>
