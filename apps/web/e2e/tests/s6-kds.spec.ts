...
test.describe('S6 — KDS', () => {
  test('dine_in sipariş → KDS\'te görün → Hazırlanıyor → Hazır', async ({
    ...
    // 3. KDS sayfasına SPA navigation ile git: page.goto('/kds') hard
    //    reload Zustand in-memory state'i sıfırlardı ve /login'e
    //    redirect ederdi. history.pushState + popstate React Router'ı
    //    tetikler (BrowserRouter popstate listener), full reload yok.
    await page.evaluate(() => {
      window.history.pushState({}, '', '/kds');
