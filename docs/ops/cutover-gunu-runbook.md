...
- [x] **🖥️ Kasiyer istasyonu — [USER kararı S98]: dükkan-PC'de Chrome tam-ekran/kiosk** (ek donanım yok; yazıcı-agent'larıyla aynı makine). **Kurulum reçetesi HAZIR: `kasiyer-kiosk-kurulum.md`** (`--app=` kısayolu + shell:startup otomatik-başlatma + güç/ekran hijyeni + oturum notu + smoke).
