...
- [ ] **🔑 Personel hesapları tam** — 22 Tem prod'da **5 kullanıcı** var: İlhan/İsmail (`admin`) · Fırat (`cashier`) · Kadir/Recep (`waiter`). **Ceren ve Sıraç hesapları AÇILMADI** → cutover'dan önce açılmalı (rol `waiter`, şifre **min 10 karakter**). Doğrulama: `SELECT username, role FROM users ORDER BY role`.
...
- [ ] *(Bloklayıcı değil, önerilir — ADR-032 Amd2 açık [USER] borcu)* **Yetim-kuyruk uyarısı gerçek olayla sınandı:** ızgara servisi kasıtla durdurulur → `grill` siparişi gönderilir → `/tanimlamalar/yazicilar`'da uyarı **yanar** → servis kaldırılır → uyarı **söner**. Sınanmazsa cutover gecesi bu göstergenin sessiz kalma ihtimali bilinmez.
...
| kind | istasyon | donanım / transport |
|---|---|---|
| `kitchen` | **FIRIN** | JP80H-UE · **doğrudan TCP `192.168.1.120:9100`** |
| `grill` | **IZGARA** | POS80 (`IZGARA2025`) · **doğrudan TCP `192.168.1.87:9100`** |
| `bill` | **KASA** | POS-80 · **USB, spooler-RAW** (Adisyo ile **paylaşımlı** — sürücüye dokunulmaz) |
