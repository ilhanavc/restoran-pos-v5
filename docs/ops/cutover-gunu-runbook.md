...
>
> **🗓️ Hedef (S98 kullanıcı kararı, 2026-07-17): ÖNÜMÜZDEKİ HAFTA (20-26 Tem 2026) içinde bir akşam.** Kesin gün, Apple-üyelik onayı + iOS kurulumunun bitişine göre sabitlenir (iki-platform birlikte başlama kararı — §0 iOS gate). Kritik yol: Apple Developer kaydının ~2-3 gün içinde tamamlanması (kimlik-kamera sorunu → alternatif: başka iPhone'dan kayıt veya web-kayıt).
...
- [ ] **🍎 iOS gate (ADR-031 Amendment 1 K3) — [USER kararı S98]: iOS BEKLENİR, pilot İKİ-PLATFORM birlikte başlar.** Zincir: Apple Developer üyeliği [USER] → `eas device:create` → ad-hoc IPA kuruldu + §11.8 iOS smoke geçti. Android-only fallback KULLANILMAYACAK (Amd1 K3'teki fallback geçerliliğini korur ama tercih edilmedi); Apple onayı gecikirse **cutover tarihi kayar**, kapsam daralmaz.
- [x] **🖥️ Kasiyer istasyonu — [USER kararı S98]: dükkan-PC'de Chrome tam-ekran/kiosk** (ek donanım yok; yazıcı-agent'larıyla aynı makine). Cutover öncesi hazırlık: kiosk kısayolu (`chrome --kiosk https://restoranpos.org`) + Windows oturum-açılışında otomatik başlatma + kasiyer oturumunun açık kalma düzeni.
...
- [ ] **[OPS + kullanıcı onayı]** Test order'ları temizle (SQL, dikkatli — yalnız test kayıtları; müşteri 1469/menü/masa KORUNUR). **[USER kararı S98]: EVET — hard-delete + `order_no` 1'den** ("temiz başlangıç"); akış değişmez: yedek → liste göster → onay → sil.
