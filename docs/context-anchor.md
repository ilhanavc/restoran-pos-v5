...
- **Phase:** 1 ✅, 1.5 ✅, 2 Sprint 0 ✅, 2 Sprint 1 ✅, 2 Sprint 2 ✅, **2 Sprint 3a (ABAC unblock + CI gating)** ✅ KAPANDI, **Sprint 3b altyapı (ADR + plan + retrospektif)** ✅ HAZIR. **Sıradaki: Görev 17.5** (Migration 006 product_variants + zod sync) → Görev 17 Users CRUD → Görev 18 Products/Variants CRUD. **3 blocker durumu:** ADR-002 §10 ✅ | ADR-003 §8.6 (Görev 18 ADR-X) ✅ | `permissions.ts` plan-kod drift ✅ RESOLVED. **Açık kritik borç:** Active-plan vs charter Phase 2 drift (Socket.IO + Web UI 7 ekran + E2E hiç plan'a girmemiş, Sprint 3b kapanışından sonra plan yeniden yapılandırılır — §2 'Açık stratejik borçlar' detayı).
...
- **Açık stratejik borçlar:**
  ...
  - **🔴 [BLOCKER Görev 18] ADR-003 §8.6 `price_delta_cents` semantik amendment:** ADR §8.6 SQL'i `INTEGER NOT NULL` yazıyor ama signed/unsigned, negatif değer izinli mi, range hard-cap var mı **açıkça tanımsız**. 2026-04-28 Görev 17.5 schema sync sırasında tespit. Mevcut zod `z.number().int()` (signed, DB INTEGER ile bire bir tutarlı, drift yok); ama Görev 18 CRUD validation `price_delta_cents` zod refine'larına dokunacak ve kararsız semantikle yazılamaz. Pratik senaryo (v3 davranış referansı): pide/porsiyon variant'ında negatif delta **gerçek kullanım** — "küçük boy -2 TL" gibi indirim varyantları. **Aksiyon:** ADR §8.6'ya 1 paragraflık amendment (signed; pozitif/sıfır/negatif izinli; range için zod safe-integer doğal sınır) + decisions.md changelog satırı; ayrı küçük PR Görev 18 başlamadan merge edilir. **Sprint 3b kapanış kriterlerine eklendi.**
