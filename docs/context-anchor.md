...
- **Phase:** 1 ✅, 1.5 ✅, 2 Sprint 0 ✅, 2 Sprint 1 ✅, 2 Sprint 2 ✅, **2 Sprint 3a (ABAC unblock + CI gating)** ✅ KAPANDI, **2 Sprint 3b (Users CRUD + ADR §8.6 amendment + Products/Variants CRUD)** ✅ KAPANDI, **Phase 2 charter drift cleanup plan revizyonu** ✅ KAPANDI (PR #39 — Sprint 4-9 dizilim + KDS Phase 3 numaralandırma). **Sıradaki: Sprint 4 (Tables + Categories full CRUD)** — sonra Sprint 5 (areas + ADR-009) → 6 (settings) → 7 (Socket.IO + ADR-010) → 8a-d (Web UI 7 ekran + ADR-011) → 9 (Playwright E2E). Tahmini ~10 hafta Phase 2 tamamlanma. **Sprint 3b ana keşif (RESOLVED PR #36):** ADR-003 §8.6 `price_delta_cents` semantik amendment (signed/negatif).
...
- **Sıradaki:** **Sprint 4 — Tables + Categories Full CRUD (PATCH/DELETE)** — Sprint 3b admin CRUD pattern'i (Görev 17/18) uygulanır. Sonra Sprint 5 (areas + ADR-009) → 6 (settings) → 7 (Socket.IO + ADR-010) → 8a-d (Web UI 7 ekran + ADR-011) → 9 (Playwright E2E). Tahmini ~10 hafta Phase 2 tamamlanma. ADR-007 (rate limiting) Phase 2 ortasında. Detaylar `.claude/plans/active-plan.md` Sprint 4-9 bölümlerinde.
- **Çalıştırma:**
