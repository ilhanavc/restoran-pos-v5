...
- **Aktif görev:** ADR-003 Bölüm 14 ✅ — Session 18 kapanışı; sıradaki Session 19 §15
  - Bölüm 1-9 onaylı ✅
  ...
  - **Bölüm 14 onaylı ✅ (Session 18, 2026-04-25 commit eddf6f7):** Kritik Index'ler — 10 alt-bölüm, ~280 satır. Konvansiyonlar: naming `<table>_<columns>[_partial-suffix]_<kind>` + partial-suffix kapalı sözlük (`_active`/`_open`/`_pending`) + §14.1.B.1 INVALID retry policy (pg_index WHERE indisvalid=false tarama, drop+retry) + §14.1.B.2 migrator-only DDL kontratı (app_admin DDL yasak) + INCLUDE üç-koşul gate (premature optimization yasak). Tablo bazlı: orders `(tenant_id, store_date, order_no)` çift-rol + tek-aktif-masa partial `_open` (NULL muafiyeti kasıtlı, pilot'ta eş zamanlı giriş yok → v5.1 guard); audit/call_logs `(tenant_id, created_at DESC)` bounded-log leading (DESC backward scan ASC eşit maliyetli; sistem-actor NULL-ucu range scan; 4. index reddi explicit-lock); order_no_counters PK muafiyeti; soft-delete partial matrix (products/categories/customers/tables `_active`; snapshot tabloları + customer_phones yasak); payments trigger+index ayrımı; customer_phones tam UNIQUE drift koruma (4 katman); print_jobs forward-ref ADR-004; §14.9 RLS+index leftmost-prefix uyumu. İkili review paralel: db-guard 0+4+2+8, security 0+2+3+6 → mini-pass A1-A6 + edge case + §14.1.B.2 "veya" netlemesi kapatıldı. 5 follow-up (B1-B5) active-plan.md'de.
  - Bölüm 15-16 henüz yazılmadı (Migration Stratejisi, Consequences — §15 sıradaki)
- **Son tamamlanan:** ADR-003 Bölüm 14 Kritik Index'ler + ikili review + mini-pass A1-A6 — Session 18 commit `eddf6f7`
- **Sıradaki görev:** Session 19 → **Bölüm 15 (Migration Stratejisi — Forward-Only + Tool Seçimi)** architect draft → db-migration-guard review → Bölüm 16 (Consequences) → ADR-003 kabul → şablon migration `apps/api/migrations/000_init.sql`
  - §15 karar yüzeyi: tool seçimi `node-pg-migrate` (runner) + `kysely` (runtime) + `kysely-codegen` (TS tip üretimi) — outline'da pre-lock'lu, §15 gerekçe + alternatif reddi yazar; forward-only (down migration yok, dev-reset local-only); drift detection CI gate (kysely-codegen diff + §14.1.B.1 INVALID index tarama); CONCURRENTLY enforcement (parser-level DDL gate); migrator-only DDL (§14.1.B.2 bağı); migration ordering (000_init sıralaması tenants → tenant_settings)
