...
- **Phase:** 1 ✅, **Phase 1.5** ✅, **Phase 2 Sprint 0** ✅, **Phase 2 Sprint 1** ✅, **Phase 2 Sprint 2 (GET endpoint'leri)** ✅ KAPANDI. **Şu an: Phase 2 Sprint 3a (Görev 15 — POST /orders waiter_user_id hotfix PR review).** Görev 14 (migration 005) PR #24 squash `a12fdcb` merged. Sprint 1 borç fix #25 squash `febc795` merged (orders.test.ts fixture'a tenant_settings INSERT). Sprint 3 plan PR #22 + ADR-003 §14.1.B.3 amendment PR #23 önceden merged. **Yeni görev: Sprint 3a Görev 15.5** — CI integration test gating (DATABASE_URL + postgres service container) Görev 16 ABAC öncesi blocker.
...
- **Açık stratejik borçlar:**
  ...
  - **CI integration test sahte yeşil drift (kritik — Görev 15.5 blocker):** `DATABASE_URL` CI'de set edilmediği için `describe.skipIf(DB_URL undefined)` ile tüm integration testler **skip** oluyor. PR #18 "16 test yeşil" iddiası CI tarafında **skip durumu**, gerçek execution değil. Etki: ADR-008, ADR-002 §10, Görev 14 hepsi sahte CI yeşilliğine güvendi; fixture/migration drift'leri yakalanmıyor (örn: orders.test.ts fixture tenant_settings eksikliği lokal'de fail, CI'de skip nedeniyle yakalanmadı, PR #25 ile düzeltildi). Düzeltme: Sprint 3a Görev 15.5 (yeni görev) — postgres service container + DATABASE_URL CI var + workflow update. Görev 16 ABAC öncesi blocker. 2026-04-27 Görev 15 sırasında tespit, errorHandler debug log ile kök neden bulundu.
