...
| 2 | m.9 yurt dışı (Almanya) aktarım hukuki teyidi + aydınlatma metninde belirtim | 🟡 PAKET HAZIR → avukat onayı/dayanak tesisi bekliyor | İşletme sahibi (hukuki) | §6. **S86 güncel teyit + kritik açık nokta:** Almanya için yeterlilik kararı YOK → uygun güvence şart; **Hetzner Türk Standart Sözleşmesi'ni imzalamayabilir** (ceza yalnız veri sorumlusuna → fallback: taahhütname+Kurul izni). `aydinlatma-metni-taslak.md` (Bölüm A.5, C, E#2, F). Kapanış = dayanak tesisi + 5 iş günü Kurul bildirimi. |
| 3 | İlgili kişilere aydınlatma (ön-bildirim) planı — m.10 unsurları | 🟡 PAKET HAZIR → avukat onayı/yayın bekliyor | İşletme sahibi | §7, §10. **S86: [ALANLAR] dolduruldu** (İlhan Avcı / Dilan Pide / Mürefte Mah., Şarköy-Tekirdağ / 0539 840 08 56). Müşteri+personel metni + sunum: `aydinlatma-metni-taslak.md` (Bölüm A, B, D). Kapanış = onay + yayın. |
...
| 12 | Hetzner AVV/DPA (m.12/3 veri işleyen sözleşmesi) durumu belgelendi | 🔴 EKSİK | İşletme sahibi | §2 — import-blocker değil ama belgelenmeli. **S86: Hetzner GDPR AVV/DPA sunar** (ToS'ta mı ayrı imza mı teyit edilmeli; m.9 güvencesiyle birlikte ele alınır). Aksiyon: `aydinlatma-metni-taslak.md` Bölüm E#3. |
...
**Session 86 güncelleme:** Taslak **execute-hazır** hâle getirildi — [ALANLAR] işletme bilgileriyle dolduruldu (İlhan Avcı / Dilan Pide) + m.9/VERBİS rejimi güncel kaynaklara karşı teyit edildi (`aydinlatma-metni-taslak.md` Bölüm C/F). **Kritik bulgu:** Almanya için yeterlilik kararı yok + **Hetzner Türk Standart Sözleşmesi'ni imzalamayabilir** (ceza yalnız veri sorumlusuna) → avukatın çözmesi gereken düğüm (fallback: taahhütname + Kurul izni). VERBİS: işletme **muaf** (<50 çalışan & <100M TL; muafiyet Kanun yükümlülüğünü kaldırmaz). #4 (yedek) S85'te ✅ kapandı. **Statü değişmedi (🟡): #2/#3 hâlâ avukat onayı + m.9 tesisi + yayın bekliyor** — paket hazırlığı bunları başlatır, kapatmaz.
